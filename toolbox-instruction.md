...
- 🚀 修改创建鸿蒙项目工具：去掉创建鸿蒙模板工程的mcp，后续会改为skills的方式提供。
- 🔨 提供一种轻量级调用mcp的方式：新版本暂时去掉了toolbox界面，用户无须配置环境变量即可使用mcp。
配置方式如下：
{
  "mcpServers": {
...
- 📚 **知识库内容更新**：新增 API 22 Release 版本参考文档、音频开发指南及版本变更预告，Toolbox 已集成云端知识库工具。
- 🚀 **新增创建鸿蒙项目工具**：支持通过 MCP 工具快速创建鸿蒙模板工程，简化项目初始化流程。
- 🔧 **LSP 插件按需下载**：ArkTS 语法高亮插件改为添加时自动下载/手动下载。
- 🎯 **语法插件与 Check 工具共享 LSP**：优化架构，语法高亮插件与 ArkTS Check 工具复用同一个语言服务器。
- 🐛 **修复 macOS 窗口拖动问题**：解决了 macOS 版本中窗口无法拖动的交互问题。
- 🔨 **修复 MCP Tools 注册问题**：修复了在无模拟器配置的情况下，部分 MCP 工具无法正常注册的问题。
...
将 Windows 的 zip 压缩包解压到本地，其中文件夹应该包含 `deveco-toolbox.exe`、`deveco-mcp-server.exe` 和 `arkts-lang-server` 目录。双击打开 `deveco-toolbox.exe` 进行配置。

> **注意**: 从 v0.1.6 版本开始，ArkTS 语法高亮插件不再包含在安装包中，使用时会自动下载最新版本。
...
> **Notice:**
> 若出现mcp可用工具缺失问题，通常由lsp初始化未完成导致，重新启动mcp即可解决。
...
> **v0.1.6 新特性**：DevEco MCP Server 现已内置云端知识库工具支持，无需单独配置即可使用鸿蒙知识库查询功能。
...
| 参数 | 必选/可选 | 描述 | 默认值 | 格式 / 取值说明 |
| :--- | :--- | :--- | :--- | :--- |
| `--projectPath` | 必选 | 项目创建路径 | 无 | `path/to/your/project` |
| `--appName` | 必选 | 应用名称 | 无 | 例如：MyApp |
| `--apiLevel` | 可选 | API Level | 17 | 17, 18, 19, 20, 21, 22 |

**使用示例:**

> ```SQL
> 1.用户输入:"创建一个api22的鸿蒙项目"
> 
> 2.执行create_harmony_project工具创建项目模板
> 
> 3.系统响应：项目创建成功！
> ✅ 鸿蒙项目创建成功！
> 
> 📁 项目位置：d:\HarmonyOS\1\2\3\5
> 📱 应用名称：HarmonyApp
> ----------------------------------------
> ⚙️ 配置详情：
>   - Target SDK Version: 5.0.5(17)
>   - Compatible SDK Version: 5.0.5(17)
>   - Model Version: 5.0.5
> ----------------------------------------
> 🔄 正在同步项目...
> ✅ 项目同步成功
...
> ----------------------------------------
> 🚀 接下来你可以：
>   1. 使用 'check_ets_files' 工具检查 ArkTS 语法
>   2. 使用 'build_project' 工具编译该项目
